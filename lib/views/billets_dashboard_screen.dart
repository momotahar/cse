...
class _BasesPanel extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final bases = stats.byBase.entries.map((e) {
      final base = e.key;
      final cseBase = e.value;
      final negosBase = stats.byBaseNegos[base] ?? 0;
      final qteBase = stats.byBaseQte[base] ?? 0;

      // Sommes dérivées depuis le détail libellé → on récupère Plein et Subv
      final libMap = stats.byBaseLibelle[base] ?? const <String, _LibelleAgg>{};
      final pleinBase = libMap.values.fold<double>(0, (s, a) => s + a.plein);
      final subvBase = libMap.values.fold<double>(
        0,
        (s, a) => s + a.subvention,
      );
      ...
      return _RowAgg(
        ...
        cse: cseBase,
        negos: negosBase,
        plein: pleinBase, // ← vrai total Plein par base
        subv: subvBase, // ← vrai total Subv par base
      );
    }).toList()..sort((a, b) => b.cse.compareTo(a.cse));
...
class _BaseTile extends StatelessWidget {
  ...
  const _BaseTile({
    ...
    required this.plein, // ← AJOUT
    required this.subv, // ← AJOUT
  });
