...
class _CommandesScreenState extends State<CommandesScreen> {
          ...
          cells: [
            ...
            DataCell(
              Tooltip(
                ...
                child: Text(_ucFirst(c.billetLibelle)),
              ),
            ),
            DataCell(Text('${c.qte}')),
            DataCell(
            ...
            DataCell(Text(c.agent)), // déjà stocké formaté
  ...
  Future<void> _deleteCommande(Commande c) async {
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text(
          'Supprimer la commande de « ${_ucFirst(c.billetLibelle)} » pour ${c.agent} ?',
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
  ...
  Future<Uint8List> _buildCommandesPdfBytes(
    ...
    // Styles compacts
    const double kHeaderSize = 9;
    const double kCellSize = 9;
    const double kTitleSize = 11;
      ...
      pw.MultiPage(
        ...
        build: (ctx) => [
          ...
          pw.Table.fromTextArray(
            ...
            cellStyle: const pw.TextStyle(fontSize: kCellSize, height: 1.05),
            cellPadding: const pw.EdgeInsets.symmetric(
              horizontal: 4,
              vertical: 5,
            ),
            border: pw.TableBorder.symmetric(
              inside: const pw.BorderSide(color: kPdfTableLine, width: 0.3),
              outside: const pw.BorderSide(color: kPdfTableLine, width: 0.6),
            ),
            columnWidths: const {
              0: pw.FlexColumnWidth(1.1), // DATE
              1: pw.FlexColumnWidth(1.4), // AGENT
              2: pw.FlexColumnWidth(1.9), // BILLET
              3: pw.FlexColumnWidth(0.7), // QTÉ
              4: pw.FlexColumnWidth(1.1), // P.U.
              5: pw.FlexColumnWidth(1.1), // MONT
              6: pw.FlexColumnWidth(1.0), // BASE
            },
            cellAlignment: pw.Alignment.centerLeft,
            cellAlignments: const {
              3: pw.Alignment.centerRight, // QTÉ
              4: pw.Alignment.centerRight, // P.U.
              5: pw.Alignment.centerRight, // MONT
            },
          ...
          pw.Container(
            ...
            decoration: const pw.BoxDecoration(
              border: pw.Border(
                top: pw.BorderSide(color: kPdfBlue, width: 1.0),
              ),
            ),
            child: pw.Row(
  ...
  /// Met la 1re lettre en majuscule, le reste inchangé.
  String _ucFirst(String s) {
    if (s.isEmpty) return s;
    return s[0].toUpperCase() + s.substring(1);
  }

  /// Met en majuscule la 1re lettre de chaque mot (espaces, tirets, apostrophes),
  /// et met le reste en minuscules pour un rendu propre type "Jean-Pierre D’Arc".
  String _ucWords(String s) {
    String cap(String p) =>
